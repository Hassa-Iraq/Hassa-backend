...
export async function create(params: CreateMenuItemParams): Promise<MenuItemRow> {
  const r = await pool.query(
    `INSERT INTO restaurant.menu_items (restaurant_id, category_id, name, description, price, image_url, is_available, display_order)
...
export async function findByRestaurantId(
  restaurant_id: string,
  opts?: { category_id?: string; limit?: number; offset?: number }
): Promise<MenuItemRow[]> {
  ...
  if (opts?.category_id) {
    ...
    q += " ORDER BY display_order ASC, created_at ASC LIMIT $3 OFFSET $4";
  } else {
    vals.push(limit, offset);
    q += " ORDER BY display_order ASC, created_at ASC LIMIT $2 OFFSET $3";
  }
