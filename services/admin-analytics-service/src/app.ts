...
const baseSwaggerSpec = createSwaggerSpec({
  serviceName: "Admin Analytics Service",
  version: "1.0.0",
  description: "Admin analytics service API",
...
app.get("/api-docs/swagger.json", (req: Request, res: Response) => {
  ...
  if (host && host.includes("localhost") && host.includes(":")) {
    apiBasePath = `http://localhost:${config.PORT || 3007}`;
  } else if (host) {
    apiBasePath = `${protocol}://${host}/api/admin`;
  } else {
    apiBasePath = `http://localhost:${config.PORT || 3007}`;
  }
...
app.use(
  ...
  (req: Request, res: Response, next: NextFunction) => {
    ...
    swaggerUi.setup(swaggerSpec, {
      ...
      customSiteTitle: "Admin Analytics Service API Documentation",
    })(req, res, next);
  }
);
