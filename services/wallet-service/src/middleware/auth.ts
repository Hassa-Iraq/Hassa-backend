...
export async function authenticate(
  ...
  try {
    ...
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      res.status(401).json({ success: false, status: "ERROR", message: "No token provided", data: null });
      return;
    }
    ...
    if (!response.ok) {
      res.status(401).json({ success: false, status: "ERROR", message: "Invalid or expired token", data: null });
      return;
    }
    ...
    res.status(401).json({ success: false, status: "ERROR", message: "Authentication failed", data: null });
  }
}
...
export function authorize(...allowedRoles: string[]) {
  return (req: AuthRequest, res: Response, next: NextFunction): void => {
    if (!req.user) {
      res.status(401).json({ success: false, status: "ERROR", message: "Authentication required", data: null });
      return;
    }
    if (!allowedRoles.includes(req.user.role)) {
      res.status(403).json({ success: false, status: "ERROR", message: "Insufficient permissions", data: null });
      return;
    }
    next();
  };
